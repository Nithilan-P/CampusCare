...
import { getAssignedComplaints } from '../../api/complaints';
...
function AssignedComplaints() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-text-primary">Assigned Complaints</h1>
        <p className="text-sm text-text-secondary">Complaints currently assigned to you.</p>
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-surface shadow-sm">
        {loading ? (
          ...
          <p className="p-6 text-sm text-text-secondary">No assigned complaints found.</p>
        ) : (
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border bg-background">
              <tr>
                <th className="px-4 py-3 font-medium text-text-secondary">Title</th>
                <th className="px-4 py-3 font-medium text-text-secondary">Reported By</th>
              ...
              {filteredComplaints.map((complaint) => (
                ...
                >
                  <td className="px-4 py-3 font-medium text-text-primary">
                    <Link to={`/staff/complaints/${complaint._id}`} className="hover:text-primary">
                      {complaint.title}
                    </Link>
                  </td>
